<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <style>
        body { font-family: sans-serif; font-size: 12px; color: #333; }
        h1 { font-size: 18px; margin-bottom: 0; }
        .subtitulo { color: #666; margin-top: 4px; margin-bottom: 20px; }
        table { width: 100%; border-collapse: collapse; margin-top: 10px; }
        th, td { border: 1px solid #ccc; padding: 6px 8px; text-align: left; font-size: 11px; }
        th { background-color: #f2f2f2; }
        .filtros { margin-bottom: 15px; font-size: 11px; color: #555; }
        .footer { margin-top: 20px; font-size: 10px; color: #999; }
    </style>
</head>
<body>

    <h1>EDESSI — Reporte de Reparaciones</h1>
    <p class="subtitulo">Generado el {{ $fechaGeneracion->format('d/m/Y H:i') }}</p>

    <div class="filtros">
        <strong>Filtros aplicados:</strong>
        Desde: {{ $filtros['fecha_desde'] ?? 'Sin filtro' }} |
        Hasta: {{ $filtros['fecha_hasta'] ?? 'Sin filtro' }} |
        Técnico: {{ !empty($filtros['tecnico_id']) ? \App\Models\Usuario::find($filtros['tecnico_id'])->nombre : 'Todos' }} |
        Estado: {{ $filtros['estado'] ?? 'Todos' }}
    </div>

    <table>
        <thead>
            <tr>
                <th>#</th>
                <th>Cliente</th>
                <th>Equipo</th>
                <th>Técnico</th>
                <th>Estado</th>
                <th>Ingreso</th>
                <th>Entrega</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($reparaciones as $r)
                <tr>
                    <td>{{ $r->id }}</td>
                    <td>{{ $r->equipo->cliente->nombre ?? '-' }}</td>
                    <td>{{ $r->equipo->tipo }} {{ $r->equipo->marca }}</td>
                    <td>{{ $r->tecnico->nombre ?? 'Sin asignar' }}</td>
                    <td>{{ ucfirst($r->estado) }}</td>
                    <td>{{ $r->fecha_ingreso->format('d/m/Y') }}</td>
                    <td>{{ $r->fecha_entrega ? $r->fecha_entrega->format('d/m/Y') : '-' }}</td>
                </tr>
            @empty
                <tr><td colspan="7">No se encontraron reparaciones con los filtros aplicados.</td></tr>
            @endforelse
        </tbody>
    </table>

    <p class="footer">Total de reparaciones en este reporte: {{ $reparaciones->count() }}</p>

</body>
</html>